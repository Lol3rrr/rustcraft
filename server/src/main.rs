...
use networking::{Connection, recv_packet, recv_legacy_packet};
...
async fn handle_connection(connection: tokio::net::TcpStream) {
    ...
    let mut connection = networking::UnencryptedConnection::new(connection);
    ...
    match packet_data.next_state {
        ...
        protocol::handshake::server::NextState::Login => {
            ...
            login(connection, buffer).await;
        }
    };
}

async fn status<S>(mut connection: S, mut buffer: bytes::BytesMut)
where
    S: Connection,
{
    loop {
...
async fn login<S>(mut connection: networking::UnencryptedConnection<S>, mut buffer: bytes::BytesMut)
where
    S: core::marker::Unpin + tokio::io::AsyncRead + tokio::io::AsyncWrite,
{
    ...
    configuration(connection, buffer).await;
}

async fn configuration<S>(mut connection: S, mut buffer: bytes::BytesMut)
where
    S: Connection,
{
    ...
    loop {
        ...
        match packet.inner {
            ...
            protocol::configuration::server::ConfigurationMessage::AckFinish(_) => {
                ...
                return play(connection, buffer).await;
            }
        };
    }
}

async fn play<S>(mut connection: S, mut buffer: bytes::BytesMut)
where
    S: Connection,
{
